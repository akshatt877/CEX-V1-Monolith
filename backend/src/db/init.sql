...
CREATE TABLE orders (
    ...
    filled_qty NUMERIC(28,8) NOT NULL DEFAULT 0.00000000, -- Fixed: default numeric value instead of string 'PENDING'
    status VARCHAR(20) NOT NULL DEFAULT 'PENDING' 
        CONSTRAINT check_order_status CHECK (status IN('PENDING','PARTIALLY_FILLED', 'FILLED', 'CANCELLED')), -- Fixed: changed semicolon to comma
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT check_qty_positive CHECK (qty > 0),
    CONSTRAINT check_filled_qty CHECK (filled_qty <= qty)
);
...
CREATE TABLE fills (
    ...
    executed_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP -- Fixed: added DEFAULT keyword
);
